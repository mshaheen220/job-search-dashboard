window.PerformanceMonitor = () => {
    ...
    if (isLoading) return (<div className="perf-loading-container"><span className="perf-loading-icon">⚙️</span><span>Loading performance metrics...</span></div>);
    if (!stats) return null;
    return (
        <div>
            <div className="perf-header">📊 Performance metrics</div>
            <div className="perf-stat-row">Cache Hit Rate: {stats.cacheStats.hitRate} (Hits: {stats.cacheStats.hits} | Misses: {stats.cacheStats.misses})</div>
            {Object.entries(stats.operationStats).slice(0, 5).map(([op, data]) => (<div key={op} className="perf-op-row">{op}: {data.avg} avg | min: {data.min} | max: {data.max} | calls: {data.count}</div>))}
        </div>
    );
};
                ...
                <div className="header-content">
                    ...
                    <div className={`header-controls ${isMenuOpen ? 'show' : ''}`}>
                        <nav className="nav">
                            ...
                            <window.Tooltip text={lastBackupTime ? `Last backup: ${lastBackupTime.toLocaleDateString()}` : "No backup yet"}><button className={`nav-btn ${backupWarning ? 'warning' : ''}`} onClick={() => handleNavClick(onBackup)}>💾 Back up {backupWarning && "⚠️"}</button></window.Tooltip>
                            <button className="nav-btn" onClick={() => handleNavClick(onImport)}>📥 Import</button>
                        </nav>
                        <div className="header-actions">
                            {window.APP_CONFIG.URLS.GITHUB && <window.Tooltip text="View on GitHub"><button className="theme-toggle theme-toggle-icon" onClick={() => window.open(window.APP_CONFIG.URLS.GITHUB, '_blank')}>
                            ...
                            <window.Tooltip text="Toggle performance metrics"><button className="theme-toggle theme-toggle-icon" onClick={() => setShowPerf(!showPerf)}>⚡</button></window.Tooltip>
